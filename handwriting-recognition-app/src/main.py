...
def capture_video(output_file='output.mp4', frame_width=640, frame_height=480, fps=20.0):
    ...
    vocab_size = len(Params.vocab) + 2  # Calculate vocab_size here
    model = HandwritingTransformer(
        input_size=16 * 24,
        vocab_size=vocab_size,  # Use the calculated vocab_size
        d_model=128,
        nhead_en=1,
        num_layers_en=1,
        nhead_de=1,
    ...
    model.load_state_dict(torch.load('D:\\JetBrains\\Projects\\handwriting_recognition\\handwriting-recognition-app\\weights\\handwriting_model.pth', map_location=device))
    model.eval()
    ...
    while True:
        ...
        out.write(frame)
        # Process the frame for handwriting detection
